import { createEdgeStyleResolver } from '@graph/render-functions/index';
...
export const useEdgeStyles = (graph: Graph, edgeId: GEdge['id']) =>
  useResolvedStyles(graph, () => {
    const resolveEdgeStyles = createEdgeStyleResolver(
      graph.theme.tokenResolver,
    );
    return resolveEdgeStyles(graph.getEdge(edgeId));
  });
