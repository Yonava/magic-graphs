...
type GetComponents = (
  controls: Pick<Controls, 'nodes' | 'edges'>,
) => CoreNode[][];
...
export type StronglyConnectedComponentsData = {
  components: CoreNode[][];
  nodeIdToComponent: NodeIdToComponent;
  /**
   * maps each component to the components adjacent to it.
   *
   * @example
   ...
   */
  componentAdjacencyMap: ComponentAdjacencyMap;
};

export const getStronglyConnectedComponentsData = (
  controls: Controls,
): StronglyConnectedComponentsData => {
  const components = getStronglyConnectedComponents(controls);

  const nodeIdToComponent = components.reduce<NodeIdToComponent>(
    (acc, scc, i) => {
  ...
  const componentAdjacencyMap = components.reduce<ComponentAdjacencyMap>(
    (acc, comp, compIndex) => {
      const componentChildren = comp
        .flatMap((node) => controls.adjacencyLists.standard()[node.id] ?? [])
        .filter((nodeId) => nodeIdToComponent.get(nodeId) !== compIndex)
        .map((nodeId) => nodeIdToComponent.get(nodeId)!);
