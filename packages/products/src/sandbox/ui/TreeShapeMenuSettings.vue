  ...
  <CPopover :offset="5">
    ...
    <GWell class="p-3 flex flex-col gap-4 w-52 rounded-md shadow-lg">
      <div>
        ...
        <InputRange v-model="options.xOffset" @update:model-value="controls.debouncedUpdateShape" :min="0"
          :max="TREE_FORMATION_OPTIONS_DEFAULTS.xOffset * 2" class="w-full" />
        <GButton v-if="showOffsetXDefaultButton" @click="setOffsetXToDefault" contrast class="text-xs">
          Set To Default
        </GButton>
      </div>
      <div>
        <h2 class="font-bold text-sm">Tree Offset Y ({{ options.yOffset }})</h2>
        <InputRange v-model="options.yOffset" @update:model-value="controls.debouncedUpdateShape" :min="0"
          :max="TREE_FORMATION_OPTIONS_DEFAULTS.yOffset * 2" class="w-full" />
        <GButton v-if="showOffsetYDefaultButton" @click="setOffsetYToDefault" contrast class="text-xs">
          Set To Default
        </GButton>
      </div>
      <div class="flex flex-col gap-2">
        <h2 class="font-bold text-lg">Shape</h2>
        <GButton @click="setShape('standard')" :secondary="options.shape === 'standard'"
          :tertiary="options.shape !== 'standard'" :disabled="options.shape === 'standard'">
          Standard
        </GButton>
        <GButton @click="setShape('binary')" :secondary="options.shape === 'binary'"
          :tertiary="options.shape !== 'binary'" :disabled="options.shape === 'binary'">
          Binary
        </GButton>
      </div>
    </GWell>
