...
<template>
  <CPopover>
    ...
    <GWell class="p-3 flex flex-col gap-2 w-72 rounded-lg">
      <div v-if="graph.nodes.value.length > 0" class="flex flex-col gap-2">
        <div class="flex justify-between">
          <h1 class="font-bold text-xl">Pick A Root Node</h1>
          <TreeShapeMenuSettings :controls="treeControls" />
        </div>

        <GWell secondary class="py-3 flex flex-wrap justify-center gap-2 max-h-48 rounded-md overflow-auto">
          <GraphNode v-for="node in graph.nodes.value" :key="node.id" @click="nodeSelected(node)" :node="node"
            :size="55" />
        </GWell>

        <div class="flex justify-between">
          <div>
            <CButton v-if="isActive" @click="deactivate" :color="colors.RED_600" class="text-xs">
              Turn Off AutoTree™
            </CButton>
            <GButton v-else @click="turnOnAutoTree" tertiary class="text-xs">
              Try AutoTree™
            </GButton>
          </div>
          <div>
            <GWell v-if="isActive && rootNode" secondary class="rounded-md px-2 py-1 font-bold text-xs animate-pulse">
